...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await fetch('https://codebrewrepo.onrender.com/api/users');
  ...
  const handleDelete = async (userId) => {
    if (confirm('Are you sure you want to delete this user?')) {
      try {
        const response = await fetch(`https://codebrewrepo.onrender.com/api/users?id=${userId}`, { method: 'DELETE' });
        if (!response.ok) {
