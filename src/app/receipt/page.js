...
const ReceiptPage = () => {
  ...
  const fetchReceipts = async () => {
    setLoading(true);
    try {
      const response = await fetch("https://codebrewrepo-1.onrender.com/api/receipt", { cache: "no-store" });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://codebrewrepo-1.onrender.com/api/receipt?id=${id}`, { method: "DELETE" });
